...
int enqueue(PROC **queue, PROC *p)
{
	...
	{//if p is not null
		...
		else
		{
			...
			trail->next = p;
			p->next = curr;
			//this handles inserting at the end and any part inbetween
...
int addOPTNode(OPT **opts, char *option)
{
	OPT *curr;
	OPT *newOpt = makeOPTNode();

	if(newOpt && option)
	{
		strcpy(newOpt->option, option);//try strcpy

		if(*opts == 0)//add to empty list
		{
			*opts = newOpt;
			return 0;
		}
...
char *getOptionAt(OPT* opts, int index)
{
	int i = 0;
	OPT *op = opts;
...
int main(int argc, char *argv[], char *env[])
{
	...
	char *opts[10] = {0};
	int count = 0;
	//OPT *opts = 0;//list of nodes that contain options
	...
	while(1)
	{
		...
		//while(opt)
		//{//get preceding options
		//	addOPTNode(&opts, opt);
		//	opt = strtok(NULL, " ");
		//	printf("->%s<-\n", opt);
		//}

		//strcpy(opt, getOptionAt(opt, 0));
		//printf("->%s<-\n", opt);
